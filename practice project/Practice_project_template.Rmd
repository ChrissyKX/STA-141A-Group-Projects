...
### Check the meaning of columns of STAR
...
data = STAR[which(!is.na(STAR$g1tmathss)) ,c("g1tmathss", "g1schid", "g1classtype", "g1tchid")]
...
We decided to use a two-way ANOVA model to fit the data. The formula of the model is as follows:

$Y_{ijk} = \mu_{..} + \alpha_{i} + \beta_{j} + \epsilon_{ijk}$
 
where \mu_{..} is the overall mean math score of all the data entries. $\alpha_i$ is the class type and can take three values: 1 for small, 2 for regular, 3 for regular with aide. $beta_j$ is the school indicator. $\epsilon_{ijk}$ is the error term explaining the random fluctuation of the observations $Y_{ijk}$ around the fitted value $\mu_{..} + \alpha_{i} + \beta_{j}$.
...
There are, at least, two possible models to use in the inferential analysis from the two prerequisite courses. 

<span style="color:blue">
We can define a two-way ANOVA model as follows 
$Y_{ijk} = \mu_{..} + \alpha_{i} + \beta_{j} + \epsilon_{ijk}$, where the index $i$ represents the class type: small ($i=1$), regular ($i=2$), regular with aide ($i=3$), and the index $j$ represents the school indicator. You need to explain the rest of the parameters, state constraints on the parameters, and justify the choice of model (e.g., why no interaction terms).  Please refer to notes from STA 106 or equivalent courses for more details.  </span>


<span style="color:blue">
You may also formulate this model in its equivalent regression form with dummy variables. The model is 
	\[
	y_i =\beta_0 + x_{i,1} \beta_1  +x_{i,2} \beta_2  +  \epsilon_i, i=1,\ldots, n,
	\]
	where $x_{i,1}$ is an indicator (0 or 1) whether the $i$th class/teacher is in a small class, and  $x_{i,1}$ is an indicator (0 or 1) whether the $i$th class/teacher is in the regular class with aide. You need to explain the rest of the parameters, state constraints on the parameters, and justify the choice of model.  Please refer to notes from STA 108 or equivalent courses for more details. 
</span>
